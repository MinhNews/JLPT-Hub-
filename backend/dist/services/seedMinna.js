...
const seedMinna = async () => {
    ...
    for (const meta of LESSON_META) {
        const existing = await MinnaLesson_1.MinnaLesson.findOne({ lessonNumber: meta.n });
        if (existing) {
            skipped++;
            continue;
        }
        const fullData = lessonDataMap[meta.n] || {};
        await MinnaLesson_1.MinnaLesson.create({
            lessonNumber: meta.n,
            level: meta.level,
            titleJp: meta.jp,
            titleVi: meta.vi,
...
exports.seedMinna = seedMinna;
// Run directly: ts-node seedMinna.ts
if (require.main === module) {
    (0, exports.seedMinna)().catch(console.error);
}
