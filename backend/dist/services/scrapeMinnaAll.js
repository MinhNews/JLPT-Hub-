...
function sanitize(content, maxLen = 120000) {
    if (!content)
        return '';
...
function parseVocab(html) {
    ...
    for (const row of rows) {
        ...
        if (!kana || ['Từ vựng', 'Kana', 'Hiragana', 'No', 'STT', ''].includes(kana))
            continue;
        if (!meaning || ['Nghĩa', 'Meaning', 'Nghĩa tiếng Việt', ''].includes(meaning))
            continue;
        if (kana.toUpperCase() === kana && kana.length < 5)
            continue;
        vocab.push({
...
function parseKanji(html) {
    ...
    for (const row of rows) {
        ...
        if (!kanjiChar || ['Hán Tự', 'Kanji', ''].includes(kanjiChar))
            continue;
        if (!hanViet || ['Âm Hán', 'Âm Hán Việt', ''].includes(hanViet))
            continue;
        kanjiList.push({
...
async function main() {
    ...
    for (let n = 1; n <= 50; n++) {
        ...
        if (Object.keys(update).length > 0) {
            try {
                const result = await MinnaLesson_1.MinnaLesson.updateOne({ lessonNumber: n }, { $set: update });
