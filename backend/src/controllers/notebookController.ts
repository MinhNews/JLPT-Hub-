...
const buildPayload = (body: any, userId: string) => {
...
export const getNotebookEntries = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const entries = await Notebook.find(filter).sort({ pinned: -1, updatedAt: -1 });
...
export const upsertNotebookEntry = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (req.body.id || req.body._id) {
      entry = await Notebook.findOneAndUpdate(
        { _id: req.body.id || req.body._id, userId },
        { $set: payload },
        { new: true }
      );
      if (!entry) return res.status(404).json({ message: 'Notebook entry not found' });
    } else if (req.body.type && req.body.originalId) {
      entry = await Notebook.findOneAndUpdate(
        { userId, type: payload.type, originalId: payload.originalId },
        { $set: payload },
        { upsert: true, new: true }
      );
    } else {
      entry = await Notebook.create(payload);
    }
...
export const updateNotebookEntry = async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user?.id;
    if (!userId) return res.status(401).json({ message: 'Unauthorized' });
...
export const patchNotebookEntry = async (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user?.id;
    if (!userId) return res.status(401).json({ message: 'Unauthorized' });
    ...
    if (patch.content !== undefined) patch.note = patch.content;
    if (patch.note !== undefined && patch.content === undefined) patch.content = patch.note;
...
export const deleteNotebookEntry = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const entry = await Notebook.findOneAndDelete({ _id: req.params.id, userId });
    if (!entry) return res.status(404).json({ message: 'Notebook entry not found' });
